...
from kts.cpd_nonlin import cpd_nonlin, seg_cpd_nonlin
...
def seg_cpd_auto(K, ncp, vmax, max_overlap, desc_rate=1, **kwargs):
    """Detect change points automatically selecting their number
    ...
    costs = scores / float(N) + penalties
    m_best = np.argmin(costs)
    cps, scatter_mtx, overlap_len, end_scatter = seg_cpd_nonlin(K, m_best, max_overlap, **kwargs)

    return cps, scatter_mtx, overlap_len,end_scatter
