...
from modifiedGoogleNet import googlenet

from kts.cpd_auto import cpd_auto, seg_cpd_auto
...
class VideoPreprocessor(object):
    ...
    def kts(self, n_frames, features):
        ...
        ncp = seq_len - 1 if seq_len - 1 < 19 else 19
        change_points, _ = cpd_auto(kernel, ncp, 2, verbose=False)
        change_points = change_points * self.sample_rate
    ...
    def seg_kts(self,features, max_overlap,vmax):
         ...
         kernel = np.matmul(features, features.T)
         ncp = seq_len - 1 if seq_len - 1 < 19 else 19
         change_points, scatter_mtx, overlap_len, last_scatter = seg_cpd_auto(kernel, ncp, vmax, max_overlap,desc_rate=self.sample_rate, verbose=False)
